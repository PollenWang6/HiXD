...
#### [ ] 5. `ContainerReader` 容器断点自适应（可选增强，非刚需）
- **价值**：★　**代价**：中
- 现状：平板双栏主从布局已由 `MainPage.ets` 的 `Navigation.mode(NavigationMode.Auto)` 白嫖系统内置 Split 满足——大屏自动左菜单+右子页，等同于系统邮件/设置/妮可之记范式，零额外代码即得良好平板体验。
- 剩余收益：仅"组件级"精细适配——折叠屏半折/分屏实时变形、侧边栏收成窄 rail、课表列数随详情区容器宽变化、抽屉/弹层内嵌页自适应。属锦上添花，已从必做降级为可选，暂不排入主线。
...
3. ~~**通知设置入口**（曾落地，2026-07-10 已移除）~~→ 改用日历导出满足课程/考试提醒需求，通知设置入口冗余，已从"我的-其他"删除（含 notificationManager 引用、菜单项 UI、两个方法）。

> 本期实际落地组合 = 编译目标切 API26 + 沉浸光感 + 若干修复（首登课表/主题色/滑块遮罩）。~~通知入口~~已移除（日历导出替代）。Tier 2/3 均为候选 backlog，非本期承诺。
...
- [x] ~~**通知引导**~~：2026-07-10 移除通知设置入口（日历导出替代），此回归项作废。
